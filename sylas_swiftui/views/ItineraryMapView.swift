...
struct ItineraryMapView: View {
    ...
    @State private var regionCoordinates = MKCoordinateRegion(
...
//                CustomNavBar()
                Map(coordinateRegion: $regionCoordinates)
                    .id(mapId)
...
//                    getRoute()
    ...
    func updateRegionCoordinates() {
        if let region = region {
            let newRegionCoordinates = MKCoordinateRegion(
                center: CLLocationCoordinate2D(latitude: region.lat, longitude: region.lon),
                span: MKCoordinateSpan(latitudeDelta: 0.05, longitudeDelta: 0.05)
            )
            regionCoordinates = newRegionCoordinates
            mapId = UUID()
        }
